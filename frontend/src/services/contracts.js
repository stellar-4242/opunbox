...
export function getMotoTokenContract(sender) {
    if (!isAddressSet(MOTO_TOKEN_ADDRESS)) {
        throw new Error('VITE_MOTO_TOKEN_ADDRESS is not configured');
    }
    const provider = providerService.getProvider(NETWORK);
    const network = providerService.getNetwork(NETWORK);
    return getCachedContract(MOTO_TOKEN_ADDRESS, OP20_ALLOWANCE_ABI, provider, network, sender);
}
export function getCasaTokenContract(sender) {
    if (!isAddressSet(CASA_TOKEN_ADDRESS)) {
        throw new Error('VITE_CASA_TOKEN_ADDRESS is not configured');
    }
    const provider = providerService.getProvider(NETWORK);
    const network = providerService.getNetwork(NETWORK);
    return getCachedContract(CASA_TOKEN_ADDRESS, OP20_ALLOWANCE_ABI, provider, network, sender);
}
export { Address };
